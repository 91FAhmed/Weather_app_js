...
const weather = async (city) => {
  const weather_fetch = await fetch(
    apiAddress + `q=${city}` + `&appid=${apiKeys}` + `&units=metric`
  );
  let data = await weather_fetch.json;
  weather_fetch.json().then(function (data) {
    document.querySelector(".city").innerHTML = data.name;
    document.querySelector(".Weather_info_data").innerHTML =
      Math.round(data.main.temp) + "°c";
    document.querySelector(".Humidity_h3 ").innerHTML =
      data.main.humidity + "%";
